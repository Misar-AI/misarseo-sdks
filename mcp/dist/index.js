...
function getRequiredEnv(name) {
    ...
    if (!value) {
        console.error(`[misarseo-mcp] Error: ${name} environment variable is required.`);
        process.exit(1);
    }
    return value;
}
/** Build and register every MisarSEO tool onto an McpServer instance. */
function buildServer(apiKey, apiUrl) {
    const client = new MisarSeoClient(apiKey, apiUrl);
    const server = new McpServer({
        name: "MisarSEO",
        version: "0.3.0",
    }, {
        instructions: "MisarSEO SEO research tools. Always call seo_list_projects first to get a projectId, then use the other tools. Some tools charge credits — proceed with focused research and ask for confirmation before large batches.",
    });
